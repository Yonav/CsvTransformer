...
class RenameHeaderTest {
    ...
    public void test() {
    // arrange & act
    new RenameHeader(Map.of("first", "first_new", "test", "second")).apply(table);

        // assert
        assertThat(table.getHeader().getCells()).hasSize(2);
        assertThat(table.getHeader().getCells().get(0)).isEqualTo("first_new");
        ...
        assertThat(table.getContent()).allMatch(row -> row.getCells().get(1).equals("second_1"));
    }
}